...
app.get('/login', function(req, resp){
    ...
    if (users[username]!=undefined){
            if (users[username].password==password){
                const payload = {username: username, email: users[username].email, forename:users[username].forename,surname:users[username].surname};
                var token = jwt.sign(payload, app.get('superSecret'),{ expiresIn: '30m' });
                resp.json({
                  success: true,
...
app.post('/signup', function(req, resp){
    ...
    if (users[username]==undefined){
        users[username] = {forename:req.body.firstname,
			  surname: req.body.lastname, email: req.body.email, password:req.body.password};
